...
class MailCampaigns_SynchronizeContacts_Model_Observer
{
    ...
    public function SynchronizeOrder(Varien_Event_Observer $observer)
    {
        try {
            ...
            if ($mcAPI->ImportOrders == 1 && $mcAPI->APIKey != "" && $mcAPI->APIToken != "" && $mcAPI->APIStoreID > 0) {
                ...
                if (isset($mc_order_data["store_id"])) {
                    $mc_data["store_id"] = $mc_order_data["store_id"];
                }
                if (isset($mc_order_data["entity_id"])) {
                    $mc_data["order_id"] = $mc_order_data["entity_id"];
                }
                if (isset($mc_order_data["increment_id"])) {
                    $mc_data["order_name"] = $mc_order_data["increment_id"];
                }
                if (isset($mc_order_data["status"])) {
                    $mc_data["order_status"] = $mc_order_data["status"];
                }
                if (isset($mc_order_data["grand_total"])) {
                    $mc_data["order_total"] = $mc_order_data["grand_total"];
                }
                if (isset($mc_order_data["customer_id"])) {
                    $mc_data["customer_id"] = $mc_order_data["customer_id"];
                }
                if (isset($mc_order_data["quote_id"])) {
                    $mc_data["quote_id"] = $mc_order_data["quote_id"];
                }
                if (isset($mc_order_data["customer_email"])) {
                    $mc_data["customer_email"] = $mc_order_data["customer_email"];
                }
                if (isset($mc_order_data["customer_firstname"])) {
                    $mc_data["firstname"] = $mc_order_data["customer_firstname"];
                }
                if (isset($mc_order_data["customer_lastname"])) {
                    $mc_data["lastname"] = $mc_order_data["customer_lastname"];
                }
                if (isset($mc_order_data["customer_middlename"])) {
                    $mc_data["middlename"] = $mc_order_data["customer_middlename"];
                }
                if (isset($mc_order_data["customer_dob"])) {
                    $mc_data["dob"] = $mc_order_data["customer_dob"];
                }
                if (isset($address["telephone"])) {
                    $mc_data["telephone"] = $address["telephone"];
                }
                if (isset($address["street"])) {
                    $mc_data["street"] = $address["street"];
                }
                if (isset($address["postcode"])) {
                    $mc_data["postcode"] = $address["postcode"];
                }
                if (isset($address["city"])) {
                    $mc_data["city"] = $address["city"];
                }
                if (isset($address["region"])) {
                    $mc_data["region"] = $address["region"];
                }
                if (isset($address["country_id"])) {
                    $mc_data["country_id"] = $address["country_id"];
                }
                if (isset($address["company"])) {
                    $mc_data["company"] = $address["company"];
                }
                if (isset($mc_order_data["created_at"])) {
                    $mc_data["created_at"] = $mc_order_data["created_at"];
                }
                if (isset($mc_order_data["updated_at"])) {
                    $mc_data["updated_at"] = $mc_order_data["updated_at"];
                }
    ...
    public function QuoteCron()
    {
        ...
        $rows = $connection_read->fetchAll($sql);
        foreach ($rows as $row) {
            $last_process_time = $row["datetime"];
        }
            ...
            if ($row["collection"] == "customer/customer") {
                ...
                foreach ($customersCollection as $customer) {
                    try {
                        ...
                        if ($tmp_customer_data["store_id"] == 0 || $tmp_customer_data["store_id"] == $mcAPI->APIStoreID) $customer_data[] = $tmp_customer_data;
                    } catch (Exception $e) {
                        $mcAPI->DebugCall($e->getMessage());
                    }
                }
                ...
                if (($currentPage + 1) > $pages) {
                    ...
                    $mcAPI->Call("update_magento_progress", $mc_import_data);
                } else // Update job if not finished
                {
            ...
            if ($row["collection"] == "sales/order") {
                ...
                foreach ($ordersCollection as $order) {
                    try {
                        ...
                        if ($mc_order_data["store_id"] == $mcAPI->APIStoreID || $mc_order_data["store_id"] == 0) {
